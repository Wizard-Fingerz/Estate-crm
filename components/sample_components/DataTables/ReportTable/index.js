...
const ReportTable = ({
  ...
  const filteredData =
    selectedCategory === "All"
      ...
      : data.filter(
        (item) =>
          item["report-type"].value === selectedCategory &&
          Object.values(item).some(
            (value) =>
              typeof value === "object" &&
              value?.value?.toLowerCase?.().includes(searchQuery.toLowerCase())
      ...
      <section className={styles["table-container"]}>
        <div className={styles["table-header"]}>
          <div>
            <div>
              <Section>
                {/* Update each ActionButton to include the isActive prop */}
                <ActionButton
                  label="All"
                  isActive={activeButton === "All"}
                  inverse={true}
                  onClick={() => { handleCategoryChange('All'); handleButtonClick('All'); }}
                  style={{ margin: '0 5px' }}
                />
                <ActionButton
                  label="Business"
                  ...
                  inverse={true}
                  onClick={() => { handleCategoryChange('Business'); handleButtonClick('Business'); }}
                  style={{ margin: '0 5px' }}
                />
                <ActionButton
                  label="Posts"
                  ...
                  inverse={true}
                  onClick={() => { handleCategoryChange('Posts'); handleButtonClick('Posts'); }}
                  style={{ margin: '0 5px' }}
                />
                <ActionButton
                  label="Users"
                  ...
                  inverse={true}
                  onClick={() => { handleCategoryChange('Users'); handleButtonClick('Users'); }}
                  style={{ margin: '0 5px' }}
                />
              </Section>
            </div>
